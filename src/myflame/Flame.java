...
public class Flame extends Thread {
    ...
    public void flameEvolve() {
        ...
        for (int pixel = 0, row = 0, col = 0;
            ...
            else if (temperature[col][row] > 60) {
                buffer[pixel + 1] = (byte) 0; // blue
                buffer[pixel + 2] = (byte) 0; // green
                buffer[pixel + 3] = (byte) 240; // red
            } //darker red color
            else if (temperature[col][row] > 30) {
                buffer[pixel + 1] = (byte) 0; // blue
                buffer[pixel + 2] = (byte) 0; // green
                buffer[pixel + 3] = (byte) 150; // red
            } //average red color
            else if (temperature[col][row] > 15) {
                buffer[pixel + 1] = (byte) 0; // blue
                buffer[pixel + 2] = (byte) 0; // green
                buffer[pixel + 3] = (byte) temperature[col][row]; // red
            } //average gray color for smoke effect
